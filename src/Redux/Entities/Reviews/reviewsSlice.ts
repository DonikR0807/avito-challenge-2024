...
const reviewsSlice = createSlice({
    ...
    initialState,
    reducers: {
        reviewsReset(state, action: PayloadAction<undefined>) {
            state.entities = [];
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(loadReviewsPage.pending, (state, action) => {
            state.status = REQUEST_STATUSES.pending;
        });

        builder.addCase(loadReviewsPage.rejected, (state, action) => {
